...
function DocumentsPanel({ data, refresh }: { data: ReturnType<DataStore['snapshot']>; refresh: () => void }) {
  ...
  return <section className="panel">
    <div className="section-actions"><div><h3>فهرس المستندات</h3><p className="muted">اربط ملف Google Drive بسجل واحد أو بعدة سجلات، ثم افتحه مباشرة من التطبيق.</p></div><button className="primary" onClick={() => { clear(); setShowForm(true) }}>إضافة مستند</button></div>
    {message && <p className="notice warning" role="alert">{message}</p>}
    {showForm && <form className="record-form document-form" onSubmit={submit}>
      ...
      <div className="form-actions"><button type="button" onClick={clear}>إلغاء</button><button className="primary" type="submit">{editingId ? 'حفظ التعديل' : 'حفظ المستند'}</button></div>
    </form>}
...
function CasesPanel({ clients, cases, sessions, refresh }: { clients: ReturnType<DataStore['snapshot']>['clients']; cases: ReturnType<DataStore['snapshot']>['cases']; sessions: ReturnType<DataStore['snapshot']>['sessions']; refresh: () => void }) {
  ...
  return <section className="panel">
    ...
    {showForm && <form className="record-form case-form" onSubmit={submit}>
      ...
      <div className="form-actions"><button type="button" onClick={clear}>إلغاء</button><button className="primary" type="submit">{editingId ? 'حفظ التعديل' : 'حفظ القضية'}</button></div>
    </form>}
